...
状态：已实现，等待人工审查。27 个中英日静态页面、对应工具模式、SEO 合约测试和跨浏览器回归均已接入；发布前仍由站点所有者做最终文案与交互确认。
...
- 中文默认路由、`/en/` 英文路由与 `/ja/` 日文路由；首次访问按浏览器语言进入对应版本，主动选择后记住偏好。
...
英文和日文页面使用相同 slug，分别以 `/en/`、`/ja/` 为前缀。三个语言版本通过 hreflang 互相声明，不另外创建 `md-to-image`、`markdown-screenshot`、`markdown-to-jpg` 等同义词 URL。
...
- 中文、英文和日文都以短句为主，但保留必要的技术术语。

## 验收标准

- 所有列出的中文、英文与日文 URL 都能直接打开、刷新和返回正确的页面标题。
